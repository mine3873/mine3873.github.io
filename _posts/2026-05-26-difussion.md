...
즉, 모델은 $x_{t}$와 $t$ 가 입력으로 주어졌을 때 $\frac{1}{\sqrt{\alpha_{t}}}\left( \mathbf{x}_{t} - \frac{\beta_{t}}{\sqrt{ 1 - \bar{\alpha}_{t} }}\epsilon \right)$ 을 예측하는 것과 같다. 더 나아가서 $\epsilon$ 을 제외한 나머지 모든 값들이 주어지기 때문에, 다음과 같다.  
