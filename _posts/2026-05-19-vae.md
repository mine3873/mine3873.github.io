...
변분 베이지안 (Variational Bayesian, VB) 방식에선 이러한 미지의 사후분포 $p(z|x)$를 알 수 없기 때문에, 가짜 분포 $q(z)$를 최대한 가깝게 만들어 근사하고자 했다. 이에 평균 장(Mean-field) 가정을 통해 각 가짜 분포 $q_{j}(z_{j})$를 업데이트를 다음과 같이 유도할 수 있었다.
...
이 논문에서는 이러한 문제를 **ELBO(Evidence Lower Bound)** 와 **Reparaameterization Trick** 을 통해 접근하였는데, 한번 살펴보자.
...
사후분포 $p_{\theta}(z|x)$에서 잠재 변수 $z$를 뽑고, 이 잠재변수 $z$를 통해서, $p_{\theta}(x|z)$에서 데이터 $x$를 뽑아낸다. 

근데 여기서 진짜 사후분포 $p_{\theta}(z|x) = \frac{p_{\theta}(x|z)p_{\theta}(z)}{p_{\theta}(x)}$는 계산할 수가 없다. 왜냐하면 분모가 $p_{\theta}(x) = \int p(z)p(x|z)\,dz$로, 존재할 수 있는 모든 $z$를 고려하여 적분을 해야한다. 

그래서 VAE에서는 이에 대한 근사값을 뱉는 **가짜 인코더 $q_{\phi}(z|x)$** 를 사용하여 $x$를 입력받아 잠재 공간의 $z$가 가질 수 있는 확률 분포를 출력한다. 즉, 가짜 인코더 $q_{\phi}(z|x)$의 파라미터을 조절함으로써, 복잡한 적분의 필요 없이 인코더 파라미터 $\phi$와 디코더 파라미터 $\theta$를 동시에 최적화하는 딥러닝 문제가 된 것이다.  

정리하자면,
* **인코더 $q_{\phi}(z|x)$** 는 데이터 $x$를 입력받아 데이터의 특징 분포를 출력하고, $\phi$는 진짜 사후 분포인 $p_{\theta}(z|x)$에 가까워지도록 갱신한다.
* **디코더 $p_{\theta}(x|z)$** 는 인코더에서 출력된 분포에서 샘플링된 잠재 변수 $z$를 입력받아 원래 데이터 $x$를 복원한다. $\theta$는 기존 데이터 $x$에 최대한 똑같이 복원되도록 갱신된다. 
...
$$
\begin{align}
    \log p_{\theta}(x^{(i)}) &= \int q_{\phi}(z|x^{(i)})\log p_{\theta}(x^{(i)})\,dz~~~\cdots \left( \int q(z|x^{(i)})\,dz = 1 \right) \\
    &= \int q_{\phi}(z|x^{(i)})\log\left[  \frac{p_{\theta}(x^{(i)},z)}{q_{\phi}(z|x^{(i)})} * \frac{q_{\phi}(z|x^{(i)})}{p_{\theta}(z|x^{(i)})} \right]\,dz \\
    &= \int q_{\phi}(z|x^{(i)})\log\left[ \frac{q_{\phi}(z|x^{(i)})}{p_{\theta}(z|x^{(i)})} \right]\,dz + \int q_{\phi}(z|x^{(i)})\log\left[ \frac{p_{\theta}(x^{(i)},z)}{q_{\phi}(z|x^{(i)})} \right] \, dz \\
    &= D_{KL}(q_{\phi}(z|x^{(i)}) || p_{\theta}(z|x^{(i)})) + \mathbf{E}_{q_{\phi}(z|x^{(i)})}[-\log q_{\phi}(z|x^{(i)}) + \log p_{\theta}(x^{(i)},z)] \\
...
$$
\log p_{\theta}(x^{(i)}) \geq \mathcal{L}(\theta,\phi;x^{(i)}) = \mathbf{E}_{q_{\phi}(z|x)}[-\log q_{\phi}(z|x) + \log p_{\theta}(x,z)]
$$

이 ELBO 식을 디코더 조건부 확률 공식($p(x,z) = p(z)p(x|z)$)을 이용해 다시 정리하면 다음과 같이 나타난다.  

$$
\begin{align}
    \mathcal{L}(\theta,\phi;x) &= \int q_{\phi}(z|x)\log\left[  \frac{p_{\theta}(x,z)}{q_{\phi}(z|x)} \right]\,dz  \\
    &= \int q_{\phi}(z|x)\log\left[ \frac{p_{\theta}(z) * p_{\theta}(x|z)}{q_{\phi}(z|x)} \right]\,dz \\
    &= \int q_{\phi}(z|x)\log\left[ \frac{p_{\theta}(z)}{q_{\phi}(z|x)} \right]\,dz + \int q_{\phi}(z|x)\log p_{\theta}(x|z)\,dz \\
    &= -\int q_{\phi}(z|x)\log\left[ \frac{q_{\phi}(z|x)}{p_{\theta}(z)} \right]\,dz + \int q_{\phi}(z|x)\log p_{\theta}(x|z)\,dz \\
\therefore \mathcal{L}(\theta,\phi;x^{(i)}) &= -D_{KL}(q_{\phi}(z|x^{(i)}) || p_{\theta}(z)) + \mathbf{E}_{q_{\phi}(z|x^{(i)})}[\log p_{\theta}(x^{(i)}|z)] \tag{3}
\end{align}
$$
...
임의의 미분가능한 함수 $g_{\phi}(\epsilon, x)$와 노이즈 변수 $\epsilon$을 도입하여, 기존 z를 다음과 같이 나타낸다.  
...
이렇게 하면? 기존 식 $\mathbf{E}_{q_{\phi}(z|x)}[f(z)]$에선 미분 대상인 $\phi$가 분포 $q_{\phi}$에 위치하여 앞서 살펴봤듯이 높은 분산이 나오지만, 새로운 식 $\mathbf{E}_{p(\epsilon)}[f(g_{\phi}(\epsilon, x))]$에선 **미분 대상 $\phi$가 기댓값 안쪽의 함수로 쫓겨난다.** 그러면 즉, 

$$
\begin{align}
    \mathbf{E}_{q_{\phi}(z|x^{(i)})}[f(z)] &= \mathbf{E}_{p(\epsilon)}[f(g_{\phi}(\epsilon, x^{(i)}))] \simeq \frac{1}{L} \sum_{l=1}^{L} f(g_{\phi}(\epsilon^{(l)}, x^{(i)})) \\
...
논문에 다양한 경우에서 각 함수 $g_{\phi}, p(\epsilon)$을 어떻게 선택해야할 지 나와있긴 하지만, 논문에서는 **잠재 변수의 분포 $z \sim p(z|x) = \mathcal{\mu, \sigma^{2}}$를 그냥 가우시안으로 퉁쳐버린다.** 그래서 $z \sim q_{\phi}(z|x)$도 가우시안이므로, $\epsilon \sim p(\epsilon) = \mathcal{N}(0, 1)$이 된다. 이를 통해서 함수 $g_{\phi}$는 다음과 같이 나타난다. 
...
$$
\begin{align}
    \int q_{\phi}(z|x^{(i)}) \log p_{\theta}(z)\,dz &= \int \mathcal{N}(z; \mu, \sigma^{2}) \log \mathcal{N}(z; 0, \mathbb{I})\,dz \\
...
$$
\begin{align}
    \int q_{\phi}(z|x^{(i)})\log q_{\phi}(z|x)\,dz &= \int \mathcal{N}(z; \mu, \sigma^{2}) \log \mathcal{N}(z; \mu, \sigma^{2}) \,dz \\
...
$$
\begin{align}
    -D_{KL}((q_{\phi}(z|x^{(i)}) || p_{\theta}(z))) &= \int q_{\phi}(z|x^{(i)})(\log p_{\theta}(z) - \log q_{\phi}(z|x^{(i)}))\,dz \\
...
다음으로 두번째 항은 앞서서 복원오차에 대한 항 $f(g_{\phi}(\epsilon, x^{i})) = \log p_{\theta}(x^{i}|g_{\phi}(\epsilon, x^{i}))$이라고 했었다. 이는 디코더가 이미지를 얼마나 잘 복원했는지를 나타낸다. 
...
$$
\begin{align}
    \log p_{\theta}(x|z) &\approx \sum_{j=1}^{J}-\frac{(x_{j} - \hat{x}_{j})^{2}}{2\sigma^{2}} \\
...
$$
\begin{align}
    \log p_{\theta}(x|z) &= \log\sum_{j=1}^{J}\hat{x}_{j}^{x_{j}}(1-\hat{x}_{j})^{(1-x_{j})} \\
...
이에 반해 (DC)GAN에선 고정된 수식이 아니라 판별모델 $D가 손실 함수의 역할을 하면서 실제 데이터 분포 $p_{\text{data}}$를 찾도록 모델 $G$를 조련(?)한다. 
